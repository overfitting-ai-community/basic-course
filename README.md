...
- 자동채색을 위해 인물의 Segmentation을 진행하였다. 
- 자동채색의 데이터셋을 얻는 데까지 진행하였고, 그 결과가 'Create Segmentation Results' 부분이다.
- 전체 이미지를 자동채색하는 것이 아니라, 더 잘 학습하기 위해 특정 클래스로 세분화하도록 하였다.
- 웹툰의 클래스는 무수히 많기 때문에 '교복' 이라는 우리가 세운 영역 기준으로 클래스를 정하였다.
- 무수히 많다는 것은, 웹툰은 무한히 상상할 수 있는 세계로, 뿔, 날개, 꼬리 등과 같은 인물이 가지는 다양한 악세사리를 포함하는 것들이다.
- Detectron2의 Weight를 가져온 이후에도, 오버피팅을 의도하였는 이유는 하나의 웹툰에 대해서 더 잘 Segmentation 하기 위함이다.
